...
import svgPaths from "./svg-delbzzpsuv";
...
/**
 * A flexible data grid cell component that displays content in various layouts within a table or grid structure.
 * Appears as a rectangular cell with a white background and bottom border, typically 59px in height for most layouts.
 * The cell adapts its internal layout based on the layout prop, displaying different combinations of icons, text,
 * interactive controls, or custom content.
 * 
 * This component is designed to be used within data grid/table rows to display structured information.
 * It supports multiple interaction patterns including links, checkboxes, radio buttons, and general click handlers.
 * 
 * IMPORTANT: The component manages checkbox and radio button state internally if not controlled via props.
 * For controlled behavior, provide checked and onCheckedChange/onRadioChange props.
 */
export interface DataGridCellLargeProps {
  layout?: 'Text' | 'Link' | 'Cell actions' | 'Single select' | 'Multi-select' | 'Two line text' | 'Swappable'; // layout mode (default: 'Text')
  style?: 'None' | 'Primary' | 'Secondary'; // text styling (default: 'Primary')
  content?: string; // main text content to display in the cell (default: 'Content')
  subcontent?: string; // secondary text displayed below content when layout is 'Two line text' (default: 'Subcontent')
  productIcon?: boolean; // whether to show a product icon (folder icon) before the content (default: true)
  productIcon32Px?: React.ReactNode | null; // custom product icon element to display (default: null)
  avatar32Px?: boolean; // whether to show a 32px circular avatar before the content (default: true)
  systemIcon?: boolean; // whether to show a system icon (20px placeholder icon) before the content (default: true)
  icon20PxReg?: React.ReactNode | null; // custom 20px icon element to display (default: null)
  focus?: boolean; // whether to display the focus ring around the cell (default: false)
  checked?: boolean; // controlled checked state for checkbox or radio button
  onCheckedChange?: (checked: boolean) => void; // callback fired when checkbox state changes in Multi-select layout
  onClick?: () => void; // general click handler for the entire cell
  onLinkClick?: () => void; // click handler specifically for Link layout
  radioValue?: string; // the value for the radio button in Single select layout (default: '')
  radioName?: string; // the name attribute for radio button grouping in Single select layout (default: 'radio-group')
  onRadioChange?: (value: string) => void; // callback fired when radio button is selected in Single select layout
  className?: string; // custom class name to override default styling
}

// ---------------------- Main Component ----------------------

export function DataGridCellLarge({ 
  className, 
  avatar32Px = true, 
  content = "Content", 
  focus = false, 
  ...
  onRadioChange
}: DataGridCellLargeProps) {
  // Internal state for checkbox/radio if not controlled
  ...
  const isTwoLineTextAndPrimary = layout === "Two line text" && style === "Primary";
  ...
  return (
    <div 
      className={className || `bg-[rgba(255,255,255,0)] relative ${isSingleSelectAndNone || isMultiSelectAndNone || isCellActionsAndNone ? "h-[59px]" : "w-[200px]"}`}
      onClick={handleCellClick}
      role={isMultiSelectAndNone ? "gridcell" : isSingleSelectAndNone ? "gridcell" : "gridcell"}
      tabIndex={0}
    >
      <div aria-hidden={isSingleSelectAndNone || isMultiSelectAndNone ? "true" : undefined} className={isCellActionsAndNone ? "content-stretch flex flex-col gap-[10px] h-full items-start pb-[14px] pt-[13px] relative" : isSingleSelectAndNone || isMultiSelectAndNone ? "absolute border-[#e0e0e0] border-b border-solid inset-0 pointer-events-none" : isTwoLineTextAndPrimary ? "content-stretch flex gap-[8px] items-start px-[8px] relative w-full" : isTextAndSecondary || isTextAndPrimary ? "content-stretch flex gap-[8px] items-start pl-[8px] pr-[16px] relative w-full" : isSwappableAndPrimary ? "content-stretch flex flex-col gap-[8px] items-start relative w-full" : "content-stretch flex flex-col gap-[8px] items-start pl-[8px] pr-[16px] relative w-full"}>
        {(isTextAndSecondary || isTextAndPrimary || isTwoLineTextAndPrimary) && productIcon && (
          <div className="content-stretch flex items-center overflow-clip pb-[14px] pt-[13px] relative shrink-0" data-name="Product icon container">
            {productIcon32Px || <Folder className="relative shrink-0 size-[32px]" size="32" />}
          </div>
        )}
        {(isTextAndSecondary || isTextAndPrimary || isTwoLineTextAndPrimary) && avatar32Px && (
          <div className="content-stretch flex items-center pb-[14px] pt-[13px] relative shrink-0" data-name="Avatar container">
        ...
        {(isTextAndSecondary || isTextAndPrimary || isTwoLineTextAndPrimary) && systemIcon && (
          <div className="content-stretch flex h-[59px] items-center overflow-clip pb-[21px] pt-[19px] relative shrink-0" data-name="Icon container">
            {icon20PxReg || (
              <div className="relative shrink-0" data-name="Placeholder">
                <Wrapper>
                  <DataGridCellLargeHelper>
        ...
        {(isTextAndSecondary || isTextAndPrimary || isTwoLineTextAndPrimary) && (
          <div className={`content-stretch flex flex-[1_0_0] h-[59px] min-h-px min-w-px overflow-clip relative ${isTwoLineTextAndPrimary ? "flex-col font-['Segoe_UI:Regular',sans-serif] items-start leading-[0] not-italic pb-px pt-[10px] text-ellipsis whitespace-nowrap" : isTextAndPrimary ? "items-start pb-[21px] pt-[18px]" : "items-center pb-[25px] pt-[23px]"}`} data-name="Content container">
            {isTextAndSecondary && <p className="flex-[1_0_0] font-['Segoe_UI:Regular',sans-serif] leading-[16px] min-h-px min-w-px not-italic overflow-hidden relative text-[#424242] text-[12px] text-ellipsis whitespace-nowrap">{content}</p>}
            {isTextAndPrimary && (
            ...
            {isTwoLineTextAndPrimary && (
              <>
                <div className="flex flex-col justify-center overflow-hidden relative shrink-0 text-[#242424] text-[14px] w-full">
                  <p className="leading-[20px] overflow-hidden">{content}</p>
                </div>
                <div className="flex flex-col justify-center overflow-hidden relative shrink-0 text-[#424242] text-[12px] w-full">
                  <p className="leading-[16px] overflow-hidden">{subcontent}</p>
                </div>
              </>
            )}
          </div>
        )}
        {(isTextAndSecondary || isTextAndPrimary || isTwoLineTextAndPrimary || isCellActionsAndNone) && focus && <DataGridCellLargeFocusRing />}
        {isLinkAndPrimary && (
          <div className="content-stretch flex h-[59px] items-start overflow-clip pb-[21px] pt-[18px] relative shrink-0 w-full" data-name="Link container">
            <a 
              href="#" 
              onClick={handleLinkClick}
              className="h-[21px] relative shrink-0 no-underline hover:underline"
              data-name="Link"
            >
        ...
        {isLinkAndPrimary && focus && <DataGridCellLargeFocusRing />}
        {isSwappableAndPrimary && <ContentPlaceholder className="bg-[#ebf3fc] h-[44px] relative shrink-0 w-full" />}
        {isSwappableAndPrimary && focus && <DataGridCellLargeFocusRing />}
      </div>
      <div aria-hidden={isLinkAndPrimary || isSwappableAndPrimary || isTextAndSecondary || isTextAndPrimary || isTwoLineTextAndPrimary || isCellActionsAndNone ? "true" : undefined} className={isSingleSelectAndNone || isMultiSelectAndNone ? "content-stretch flex h-full items-start pb-[22px] pl-[16px] pr-[8px] pt-[21px] relative" : "absolute border-[#e0e0e0] border-b border-solid inset-0 pointer-events-none"}>
        {isSingleSelectAndNone && (
          <div className="content-stretch flex items-center justify-center relative shrink-0 size-[16px]" data-name="Radio container">
            <label className="relative shrink-0 size-[32px] cursor-pointer" data-name="Radio">
              <input
                type="radio"
                name={radioName}
                value={radioValue}
        ...
        {isSingleSelectAndNone && focus && <DataGridCellLargeFocusRing />}
        {isMultiSelectAndNone && (
          <div className="relative shrink-0 size-[16px]" data-name="Checkbox container">
            <label className="absolute left-[-8px] top-[-8px] cursor-pointer" data-name="Checkbox">
              <input
                type="checkbox"
                checked={isChecked}
        ...
        {isMultiSelectAndNone && focus && <DataGridCellLargeFocusRing />}
      </div>
    </div>
  );
}
...
function FolderHelper1({ children }: React.PropsWithChildren<{}>) {
  return (
    <svg fill="none" preserveAspectRatio="none" viewBox={is256AndNotItemCountAndNone ? "0 0 224 144" : is16AndNotItemCountAndNone ? "0 0 16 10" : is20AndNotItemCountAndNone ? "0 0 18 12" : is24AndNotItemCountAndNone ? "0 0 22 14" : is32AndNotItemCountAndNone ? "0 0 28 18" : is40AndNotItemCountAndNone ? "0 0 36 23" : is48AndNotItemCountAndNone ? "0 0 42 27" : "0 0 84 54"} className="block size-full">
      {children}
    </svg>
  );
}

function DataGridCellLargeHelper({ children }: React.PropsWithChildren<{}>) {
  return (
...
function DataGridCellLargeFocusRing() {
  return (
    ...
    <div className={className || "relative"}>
          ...
          <div className={`absolute top-1/4 ${is16AndNotItemCountAndNone ? "bottom-[12.5%] left-0 right-0" : is20AndNotItemCountAndNone ? "bottom-[15%] left-[5%] right-[5%]" : is24AndNotItemCountAndNone ? "bottom-[16.67%] left-[4.17%] right-[4.17%]" : is40AndNotItemCountAndNone ? "bottom-[17.5%] left-[5%] mix-blend-multiply right-[5%]" : "bottom-[18.75%] left-[6.25%] right-[6.25%]"}`} data-name="Path">
            <FolderHelper1>
              {(is96AndItemCountAndNone || is96AndNotItemCountAndNone || is48AndNotItemCountAndNone || is32AndNotItemCountAndNone || is24AndNotItemCountAndNone || is20AndNotItemCountAndNone || is16AndNotItemCountAndNone || is256AndNotItemCountAndNone) && (
                <>
                  <path d={is256AndNotItemCountAndNone ? svgPaths.p3787a700 : is16AndNotItemCountAndNone ? svgPaths.p2ec72b00 : is20AndNotItemCountAndNone ? svgPaths.p29b79000 : is24AndNotItemCountAndNone ? svgPaths.p143535c0 : is32AndNotItemCountAndNone ? svgPaths.p1355f710 : is48AndNotItemCountAndNone ? svgPaths.p17dd3800 : svgPaths.p2035980} fill={is256AndNotItemCountAndNone ? "url(#paint0_linear_0_314)" : is16AndNotItemCountAndNone ? "url(#paint0_linear_0_291)" : is20AndNotItemCountAndNone ? "url(#paint0_linear_0_356)" : is24AndNotItemCountAndNone ? "url(#paint0_linear_0_296)" : is32AndNotItemCountAndNone ? "url(#paint0_linear_0_340)" : is48AndNotItemCountAndNone ? "url(#paint0_linear_0_308)" : "url(#paint0_linear_0_320)"} id="Path" />
                  <defs>
                    <linearGradient gradientUnits="userSpaceOnUse" id={is256AndNotItemCountAndNone ? "paint0_linear_0_314" : is16AndNotItemCountAndNone ? "paint0_linear_0_291" : is20AndNotItemCountAndNone ? "paint0_linear_0_356" : is24AndNotItemCountAndNone ? "paint0_linear_0_296" : is32AndNotItemCountAndNone ? "paint0_linear_0_340" : is48AndNotItemCountAndNone ? "paint0_linear_0_308" : "paint0_linear_0_320"} x1="0" x2="0" y1="0" y2={is256AndNotItemCountAndNone ? "144" : is16AndNotItemCountAndNone ? "10" : is20AndNotItemCountAndNone ? "12" : is24AndNotItemCountAndNone ? "14" : is32AndNotItemCountAndNone ? "18" : is48AndNotItemCountAndNone ? "27" : "54"}>
              ...
              {is40AndNotItemCountAndNone && (
                <>
                  <g id="Path" style={{ mixBlendMode: "multiply" }}>
                    <path d={svgPaths.p1b75b200} fill="url(#paint0_linear_0_302)" />
                  </g>
                  <defs>
                    <linearGradient gradientUnits="userSpaceOnUse" id="paint0_linear_0_302" x1="0" x2="0" y1="0" y2="23">
                      <stop stopColor="white" stopOpacity="0.01" />
      ...
      {is256AndNotItemCountAndFilled && (
        <>
          ...
          <FolderPath1>
            <path d={svgPaths.p3787a700} fill="url(#paint0_linear_0_314)" id="Path" />
            <defs>
              <linearGradient gradientUnits="userSpaceOnUse" id="paint0_linear_0_314" x1="0" x2="0" y1="0" y2="144">
                <stop stopColor="white" stopOpacity="0.01" />
                <stop offset="0.999" stopColor="#FFD75E" stopOpacity="0.3" />
              </linearGradient>
            </defs>
      ...
      {is96AndNotItemCountAndThumbnail && (
        <>
          ...
          <FolderPath2>
            <path d={svgPaths.p89a3af2} fill="url(#paint0_linear_0_300)" id="Path" />
            <defs>
              <linearGradient gradientUnits="userSpaceOnUse" id="paint0_linear_0_300" x1="0" x2="0" y1="0" y2="27">
                <stop stopColor="white" stopOpacity="0.01" />
                <stop offset="0.999" stopColor="#FFD75E" stopOpacity="0.3" />
              </linearGradient>
            </defs>
        ...
        <div className="absolute inset-[16.67%_5.56%]" data-name="folder">
          <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 32 24">
            <g id="folder">
              ...
              <path d={svgPaths.p29fa1280} fill="url(#paint0_linear_0_273)" id="Path_3" />
            ...
            <defs>
              <linearGradient gradientUnits="userSpaceOnUse" id="paint0_linear_0_273" x1="0" x2="0" y1="3" y2="24">
                <stop stopColor="white" stopOpacity="0.01" />
                <stop offset="0.999" stopColor="#FFD75E" stopOpacity="0.3" />
              </linearGradient>
            </defs>
...
interface DataGridCellLargeProps_Display {
  layout?: 'Text' | 'Link' | 'Cell actions' | 'Single select' | 'Multi-select' | 'Two line text' | 'Swappable';
...
function DataGridCellLarge_Display({
  layout,
  style,
  content,
  subcontent,
  ...
  radioValue,
  radioName,
}: DataGridCellLargeProps_Display) {
  ...
  return (
    <DataGridCellLarge
      ...
      focus={focus}
      checked={layout === 'Multi-select' ? checked : layout === 'Single select' ? selectedRadio === radioValue : undefined}
      onCheckedChange={layout === 'Multi-select' ? setChecked : undefined}
      onClick={layout === 'Cell actions' ? () => {} : undefined}
      onLinkClick={layout === 'Link' ? () => {} : undefined}
      radioValue={radioValue}
      radioName={radioName}
      onRadioChange={layout === 'Single select' ? setSelectedRadio : undefined}
    />
  );
}
...
// @figmaExample PrimaryTextDocumentCell
export function PrimaryTextDocumentCell() {
  return (
    <DataGridCellLarge_Display
      layout="Text"
      style="Primary"
...
// @figmaExample PrimaryLinkCell
export function PrimaryLinkCell() {
  return (
    <DataGridCellLarge_Display
      layout="Link"
      style="Primary"
...
// @figmaExample TwoLineProjectFilesCell
export function TwoLineProjectFilesCell() {
  return (
    <DataGridCellLarge_Display
      layout="Two line text"
      style="Primary"
      content="Project Files"
      subcontent="Modified 2 hours ago"
      productIcon={true}
      avatar32Px={false}
      systemIcon={false}
    />
...
// @figmaExample MultiSelectCell
export function MultiSelectCell() {
  return (
    <DataGridCellLarge_Display
      layout="Multi-select"
      style="None"
      content="Select this item"
      productIcon={false}
      avatar32Px={false}
      systemIcon={false}
    />
...
// @figmaExample SingleSelectOptionCell
export function SingleSelectOptionCell() {
  return (
    <DataGridCellLarge_Display
...
// @figmaExample SecondaryTextCell
export function SecondaryTextCell() {
  return (
    <DataGridCellLarge_Display
      layout="Text"
      style="Secondary"
      content="Secondary style text"
      productIcon={false}
      avatar32Px={true}
      systemIcon={false}
    />
...
// @figmaExample FocusedPrimaryTextCell
export function FocusedPrimaryTextCell() {
  return (
    <DataGridCellLarge_Display
      layout="Text"
      style="Primary"
      content="Focused cell"
      focus={true}
      productIcon={true}
      avatar32Px={false}
      systemIcon={true}
...
// @figmaExample CellActionsCell
export function CellActionsCell() {
  return (
    <DataGridCellLarge_Display
      layout="Cell actions"
      style="None"
